...
@implementation AppconomySDKExampleAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_appconomysdkexampleapp);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any AppconomySDKExampleApp patches in JS before a context is loaded... nice for 
...
{
	// create AppconomySDKExampleApp global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_appconomysdkexampleapp = [[AppconomySDKExampleAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_appconomysdkexampleapp];
	
	NSString *_appconomysdkexampleappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _appconomysdkexampleappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_appconomysdkexampleapp valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.appconomysdkexampleapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
